...
function Header() {
  ...
  const styleMenu = {  cursor:'pointer' ,margin:'15px'};
  ...
  const colorMode = React.useContext(ColorModeContext);
    ...
    return (
      ...
      <Container maxWidth="xl">
        ...
        <Box sx={{ flexGrow:'30px', display: { xs: 'flex', md: 'none' } }}>
            ...
            >
              ...
              <Typography sx={{margin:1,cursor:'pointer'}}  onClick={()=>{handleCloseNavMenu();setRoute(CATALOG_ROUTE)}}>
                  Каталог
                </Typography>
              
                <Typography  sx={{margin:1,cursor:'pointer'}} onClick={()=>{handleCloseNavMenu();setRoute(ABOUT_ROUTE)}}>
                  О нас
                  </Typography>
                        
                <Typography  sx={{margin:1,cursor:'pointer'}} onClick={()=>{handleCloseNavMenu();setRoute(CONTACT_ROUTE)}}>
                  Контакты
                  </Typography>
                                        
                <Typography  sx={{margin:1,cursor:'pointer'}} onClick={()=>{handleCloseNavMenu();setRoute(CART_ROUTE)}}>
                 Корзина
                 </Typography>
      ...
      <Box>{!matches ? <Typography sx={{color: '#7FFF00',marginLeft:15, cursor:'pointer'}} onClick={
                   ()=>setRoute(HOME_PAGE)}>climatesite.shop</Typography> : <Box sx={{display:'flex',alignItems:'center'}}>
        <AdbIcon sx={{color: '#7FFF00'}}/><Typography variant="h6"  sx={{ minWidth: 100,color: '#7FFF00',marginRight:10 , cursor:'pointer'}} onClick={
...
<IconButton  onClick={colorMode.toggleColorMode} color="inherit">
      ...
      <Box  onClick={()=>setRoute(CART_ROUTE)} sx={{display:'flex', cursor:'pointer'}}>
          ...
          <Box >
            ...
            <MenuItem onClick={()=>{handleCloseNavMenu();setRoute(AUTH_ROUTE)}}>
                ...
                {isAuth ? <MenuItem onClick={()=>{handleCloseUserMenu();localStorage.clear()}}>
